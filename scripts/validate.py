...
from red.normalize import normalize_samples, Normalizer
...
def validation_samples_iter(benign_path, malicious_samples, benign_field=None, max_benign=None,
                            normalize_benign=False):
    """Yield validation samples: benign first, then malicious.

    normalize_benign phải KHỚP với lúc train (đối xứng) — nếu không model nhận sai phân phối.
    """
    nz = Normalizer() if normalize_benign else None
    for sample in benign_samples_iter(benign_path, benign_field, max_samples=max_benign):
        yield nz.normalize(sample) if nz else sample
...
def main():
    ...
    parser.add_argument("--data-split", type=str, default="valid",
                        choices=["valid", "test"],
                        help="Which data split to validate on. "
                             "valid (default): benign_valid + events_valid_dir + evasions_valid_dir. "
                             "test: benign_test + events_test_dir + evasions_test_dir. "
                             "Backward compat: fallback to benign_valid/events_dir/evasions_dir if new fields missing.")
    ...
    if args.config:
        ...
        # ── Data split selection (valid vs test) ──
        # Fallback chain: benign_test → benign_valid → benign_train (backward compat)
        # Tương tự cho events_*_dir và evasions_*_dir
        if args.data_split == "test":
            benign_field_key = "benign_test"
            events_field_key = "events_test_dir"
            evasions_field_key = "evasions_test_dir"
        else:
            benign_field_key = "benign_valid"
            events_field_key = "events_valid_dir"
            evasions_field_key = "evasions_valid_dir"

        args.benign_samples = (
            args.benign_samples
            or data_cfg.get(benign_field_key)
            or data_cfg.get("benign_valid")
            or data_cfg.get("benign_train")
        )
        args.events_dir = (
            args.events_dir
            or data_cfg.get(events_field_key)
            or data_cfg.get("events_dir")
        )
        args.evasions_dir = (
            data_cfg.get(evasions_field_key)
            or data_cfg.get("evasions_dir")
        )
        ...
        # phải KHỚP train (đối xứng tiền xử lý benign)
        args.normalize_benign = cfg.get("training", {}).get("normalize_benign", False)
    ...
    logger.info("Data split: %s | benign=%s | events_dir=%s | evasions_dir=%s",
                args.data_split, args.benign_samples, args.events_dir, args.evasions_dir)
    ...
    feature_vectors = vectorizer.transform(
        validation_samples_iter(args.benign_samples, malicious_normalized, args.benign_field,
                                max_benign, normalize_benign=getattr(args, "normalize_benign", False))
    )
