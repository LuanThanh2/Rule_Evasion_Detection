...
def training_samples_iter(benign_path, malicious_samples, benign_field=None, max_benign=None,
    ...
    normalize_benign: nếu True, chuẩn hoá benign GIỐNG malicious (đối xứng). Mặc định
    False để giữ hành vi cũ (Windows). Bật cho Linux ART → model học ngữ nghĩa thật,
    không ăn gian theo bất đối xứng tiền xử lý.
    """
...
def main():
    ...
    if args.config:
        ...
        args.normalize_benign = train_cfg.get("normalize_benign", False)
    ...
    norm_benign = getattr(args, "normalize_benign", False)
    if norm_benign:
        logger.info("normalize_benign=True → benign chuẩn hoá ĐỐI XỨNG với malicious")
